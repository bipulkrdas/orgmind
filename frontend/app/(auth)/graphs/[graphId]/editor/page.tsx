...
import { submitEditorContent } from '@/lib/api/documents';
...
export default function EditorPage() {
  ...
  const handleSubmit = async (plainText: string, lexicalState: string) => {
    try {
      setError(null);
      ...
      // Submit both plain text and Lexical state with the graph context
      await submitEditorContent(plainText, lexicalState, graphId);
      ...
      throw err; // Re-throw so LexicalEditor can handle it
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <LexicalEditor onSubmit={handleSubmit} />
        </div>
      </div>
    </div>
  );
